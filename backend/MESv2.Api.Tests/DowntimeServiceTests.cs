...
public class DowntimeServiceTests
{
    ...
    public async Task CreateDowntimeEvent_AutoGenerated_CreatesAnnotation()
    {
        ...
        Assert.Null(result.CountsAsDowntime);
    ...
    [Fact]
    public async Task GetDowntimeEvents_ReturnsEventsForWorkCenter()
    {
        var (db, sut, _, reasonId, wcplId) = SetupWithReasons();
        var wcpl = await db.WorkCenterProductionLines.FindAsync(wcplId);

        await sut.CreateDowntimeEventAsync(new CreateDowntimeEventDto
        {
            WorkCenterProductionLineId = wcplId,
            OperatorUserId = TestHelpers.TestUserId,
        ...
        var events = await sut.GetDowntimeEventsAsync(wcpl!.WorkCenterId, null, null);

        Assert.Single(events);
        Assert.Equal("Breakdown", events[0].DowntimeReasonName);
        Assert.Equal("Equipment", events[0].DowntimeReasonCategoryName);
        Assert.True(events[0].CountsAsDowntime);
        Assert.False(string.IsNullOrEmpty(events[0].WorkCenterName));
        Assert.False(string.IsNullOrEmpty(events[0].ProductionLineName));
    }

    [Fact]
    public async Task GetDowntimeEvents_FiltersbyDateRange()
    {
        var (db, sut, _, reasonId, wcplId) = SetupWithReasons();
        var wcpl = await db.WorkCenterProductionLines.FindAsync(wcplId);
        ...
        await sut.CreateDowntimeEventAsync(new CreateDowntimeEventDto
        {
            WorkCenterProductionLineId = wcplId,
            OperatorUserId = TestHelpers.TestUserId,
            StartedAt = yesterday.AddHours(-2),
            EndedAt = yesterday.AddHours(-1),
            DowntimeReasonId = reasonId,
            IsAutoGenerated = false
        }, TestHelpers.TestUserId);
        ...
        await sut.CreateDowntimeEventAsync(new CreateDowntimeEventDto
        {
            WorkCenterProductionLineId = wcplId,
            OperatorUserId = TestHelpers.TestUserId,
            StartedAt = today.AddMinutes(-30),
            EndedAt = today.AddMinutes(-20),
            DowntimeReasonId = reasonId,
            IsAutoGenerated = false
        }, TestHelpers.TestUserId);

        var filteredEvents = await sut.GetDowntimeEventsAsync(wcpl!.WorkCenterId, today.Date, null);
    ...
    [Fact]
    public async Task UpdateDowntimeEvent_ChangesFieldsAndRecalculatesDuration()
    {
        var (db, sut, categoryId, reasonId, wcplId) = SetupWithReasons();

        var created = await sut.CreateDowntimeEventAsync(new CreateDowntimeEventDto
        {
            WorkCenterProductionLineId = wcplId,
            OperatorUserId = TestHelpers.TestUserId,
            StartedAt = new DateTime(2026, 2, 22, 10, 0, 0, DateTimeKind.Utc),
            EndedAt = new DateTime(2026, 2, 22, 10, 10, 0, DateTimeKind.Utc),
            DowntimeReasonId = reasonId,
            IsAutoGenerated = false
        }, TestHelpers.TestUserId);
        ...
        var updated = await sut.UpdateDowntimeEventAsync(created.Id, new UpdateDowntimeEventDto
        {
            OperatorUserId = TestHelpers.TestUserId,
            StartedAt = new DateTime(2026, 2, 22, 10, 0, 0, DateTimeKind.Utc),
            EndedAt = new DateTime(2026, 2, 22, 10, 25, 0, DateTimeKind.Utc),
            DowntimeReasonId = newReason.Id
        });

        Assert.NotNull(updated);
        Assert.Equal(25m, updated!.DurationMinutes);
        Assert.Equal("Material Shortage", updated.DowntimeReasonName);
    }

    [Fact]
    public async Task UpdateDowntimeEvent_ReturnsNullForMissingId()
    {
        await using var db = TestHelpers.CreateInMemoryContext();
        var sut = new DowntimeService(db);

        var result = await sut.UpdateDowntimeEventAsync(Guid.NewGuid(), new UpdateDowntimeEventDto
        {
            OperatorUserId = TestHelpers.TestUserId,
    ...
    [Fact]
    public async Task DeleteDowntimeEvent_RemovesEventAndLinkedAnnotations()
    {
        var (db, sut, _, _, wcplId) = SetupWithReasons();
        ...
        var annotationExists = await db.Annotations.AnyAsync(a => a.DowntimeEventId == created.Id);
        Assert.True(annotationExists);

        var deleted = await sut.DeleteDowntimeEventAsync(created.Id);

        Assert.True(deleted);
        Assert.Null(await db.DowntimeEvents.FindAsync(created.Id));
        Assert.False(await db.Annotations.AnyAsync(a => a.DowntimeEventId == created.Id));
    }

    [Fact]
    public async Task DeleteDowntimeEvent_ReturnsFalseForMissingId()
    {
        await using var db = TestHelpers.CreateInMemoryContext();
        var sut = new DowntimeService(db);

        var result = await sut.DeleteDowntimeEventAsync(Guid.NewGuid());

        Assert.False(result);
    }

    [Fact]
    public async Task CreateDowntimeEvent_ReturnsWorkCenterAndProductionLineNames()
    {
        var (db, sut, _, reasonId, wcplId) = SetupWithReasons();
        ...
        Assert.False(string.IsNullOrEmpty(result.WorkCenterName));
        Assert.False(string.IsNullOrEmpty(result.ProductionLineName));
    }
}
