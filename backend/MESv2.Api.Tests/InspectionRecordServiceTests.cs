...
public class InspectionRecordServiceTests
{
    ...
    [Fact]
    public async Task Create_AutoMarksDefectRepaired_ForLongSeamInspection()
    {
        await using var db = TestHelpers.CreateInMemoryContext();
        SeedSerialAndProdRecord(db, "SN-LSI-REPAIR", TestHelpers.wcLongSeamId);

        var defectCodeId = Guid.NewGuid();
        var characteristicId = Guid.NewGuid();
        var locationId = Guid.NewGuid();
        db.DefectCodes.Add(new DefectCode { Id = defectCodeId, Code = "DL1", Name = "Defect LSI" });
        db.Characteristics.Add(new Characteristic { Id = characteristicId, Code = "CL1", Name = "Char LSI", ProductTypeId = null });
        db.DefectLocations.Add(new DefectLocation { Id = locationId, Code = "LL1", Name = "Loc LSI", CharacteristicId = characteristicId });
        await db.SaveChangesAsync();

        var sut = new InspectionRecordService(db);
        var dto = new CreateInspectionRecordDto
        {
            SerialNumber = "SN-LSI-REPAIR",
            WorkCenterId = TestHelpers.wcLongSeamInspId,
            OperatorId = TestHelpers.TestUserId,
        ...
        var defect = await db.DefectLogs.SingleAsync(d => d.DefectCodeId == defectCodeId);
        Assert.True(defect.IsRepaired);
        Assert.Equal(TestHelpers.TestUserId, defect.RepairedByUserId);
        Assert.NotNull(defect.RepairedDateTime);
    }

    [Fact]
    public async Task Create_AutoMarksDefectRepaired_ForRoundSeamInspection()
    {
        await using var db = TestHelpers.CreateInMemoryContext();
        SeedSerialAndProdRecord(db, "SN-RSI-REPAIR", TestHelpers.wcRoundSeamId);

        var defectCodeId = Guid.NewGuid();
        var characteristicId = Guid.NewGuid();
        var locationId = Guid.NewGuid();
        db.DefectCodes.Add(new DefectCode { Id = defectCodeId, Code = "DR1", Name = "Defect RSI" });
        db.Characteristics.Add(new Characteristic { Id = characteristicId, Code = "CR1", Name = "Char RSI", ProductTypeId = null });
        db.DefectLocations.Add(new DefectLocation { Id = locationId, Code = "LR1", Name = "Loc RSI", CharacteristicId = characteristicId });
        await db.SaveChangesAsync();

        var sut = new InspectionRecordService(db);
        var dto = new CreateInspectionRecordDto
        {
            SerialNumber = "SN-RSI-REPAIR",
