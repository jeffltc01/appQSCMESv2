...
public class DowntimeService : IDowntimeService
{
    ...
    public async Task<IReadOnlyList<DowntimeEventDto>> GetDowntimeEventsAsync(Guid workCenterId, DateTime? from, DateTime? to, CancellationToken cancellationToken = default)
    {
        var query = _db.DowntimeEvents
            .Include(e => e.WorkCenterProductionLine)
                .ThenInclude(wcpl => wcpl.WorkCenter)
            .Include(e => e.WorkCenterProductionLine)
                .ThenInclude(wcpl => wcpl.ProductionLine)
            .Include(e => e.OperatorUser)
            .Include(e => e.DowntimeReason)
                .ThenInclude(r => r!.DowntimeReasonCategory)
            .Where(e => e.WorkCenterProductionLine.WorkCenterId == workCenterId);
        ...
        return await query
            ...
            {
                Id = e.Id,
                WorkCenterProductionLineId = e.WorkCenterProductionLineId,
                WorkCenterName = e.WorkCenterProductionLine.WorkCenter.Name,
                ProductionLineName = e.WorkCenterProductionLine.ProductionLine.Name,
                OperatorUserId = e.OperatorUserId,
                OperatorName = e.OperatorUser.DisplayName,
                DowntimeReasonId = e.DowntimeReasonId,
                DowntimeReasonName = e.DowntimeReason != null ? e.DowntimeReason.Name : null,
                DowntimeReasonCategoryName = e.DowntimeReason != null ? e.DowntimeReason.DowntimeReasonCategory.Name : null,
                CountsAsDowntime = e.DowntimeReason != null ? e.DowntimeReason.CountsAsDowntime : null,
    ...
    public async Task<DowntimeEventDto?> UpdateDowntimeEventAsync(Guid id, UpdateDowntimeEventDto dto, CancellationToken cancellationToken = default)
    {
        var entity = await _db.DowntimeEvents.FindAsync(new object[] { id }, cancellationToken);
        if (entity == null) return null;

        entity.OperatorUserId = dto.OperatorUserId;
        entity.StartedAt = dto.StartedAt;
        entity.EndedAt = dto.EndedAt;
        entity.DowntimeReasonId = dto.DowntimeReasonId;
        entity.DurationMinutes = Math.Round((decimal)(dto.EndedAt - dto.StartedAt).TotalMinutes, 2);
    ...
    public async Task<bool> DeleteDowntimeEventAsync(Guid id, CancellationToken cancellationToken = default)
    {
        var entity = await _db.DowntimeEvents
            .Include(e => e.Annotations)
            .FirstOrDefaultAsync(e => e.Id == id, cancellationToken);
        if (entity == null) return false;

        if (entity.Annotations.Any())
            _db.Annotations.RemoveRange(entity.Annotations);

        _db.DowntimeEvents.Remove(entity);
        await _db.SaveChangesAsync(cancellationToken);
        return true;
    }

    private async Task<DowntimeEventDto> MapToDto(DowntimeEvent entity, CancellationToken cancellationToken)
    {
        var wcpl = await _db.WorkCenterProductionLines
            .Include(x => x.WorkCenter)
            .Include(x => x.ProductionLine)
            .FirstOrDefaultAsync(x => x.Id == entity.WorkCenterProductionLineId, cancellationToken);

        var operatorUser = await _db.Users.FindAsync(new object[] { entity.OperatorUserId }, cancellationToken);
        DowntimeReason? reason = entity.DowntimeReasonId.HasValue
            ? await _db.DowntimeReasons
                .Include(r => r.DowntimeReasonCategory)
                .FirstOrDefaultAsync(r => r.Id == entity.DowntimeReasonId.Value, cancellationToken)
            : null;
        ...
        return new DowntimeEventDto
        {
            ...
            CountsAsDowntime = reason?.CountsAsDowntime,
